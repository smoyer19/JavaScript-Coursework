...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that if the Displlay_Valaue does not contain a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    if (First_Operand == null) {
        ...
        //in the Perform_Calculation object and the function that atches the
        //operator is executed
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //here we add a fixed amount of numbers after the decimal
        result = Number(result).toFixed(9)
        //this will remove any trailing 0's
        result = (result * 1).toStriing()
...
function Update_Display() {
    const display = document.querySelector('.caclulator-screen');
